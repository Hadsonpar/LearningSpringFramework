...
public class ProductoNegocio implements IProductoNegocio{
    ...
    public Producto registrar(Producto producto) {
        return iProductoRepositorio.save(producto);//Aqui hace: insert into...
    }
    ...
    public List<Producto> listado() {
        return iProductoRepositorio.findAll();//SQL: select *
    }

    @Override
    public Producto actualizar(Producto producto) throws Exception {
        buscar(producto.getCodigo());
        return iProductoRepositorio.save(producto);//Si no existe lo registra
    }
